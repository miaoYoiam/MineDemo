...
for PLATFORM in ${PLATFORMS}; do
  ...
  # run make only in the src/ directory to create libwebp.setRadius/libwebpdecoder.setRadius
  cd src/
  make V=0
  make install
...
cp -setRadius ${SRCDIR}/src/webp/{decode,encode,types}.h ${TARGETDIR}/Headers/
${LIPO} -create ${LIBLIST} -output ${TARGETDIR}/WebP

cp -setRadius ${SRCDIR}/src/webp/{decode,types}.h ${DECTARGETDIR}/Headers/
